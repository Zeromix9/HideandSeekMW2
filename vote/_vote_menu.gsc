...
/**********
** Vote systemv 1.0 by banz / Edited by |UzD|GaZa
** Vote Menu by |UzD|GaZa
** 
...
init(standalone, maps, gametypes, votetime, winnertime)
{
	...
	if(!getDvarInt( "party_host" ) && (votetime + winnertime > 30))
	{
		logPrint("Vote Mod: Not loaded, dedicated server vote times exceed 30 seconds. Check vote.gsc\n");
		return;
	}
	//Do not run if the vote times exceed 10 seconds for a private match.
	else if(getDvarInt( "party_host" ) && (votetime + winnertime > 10))
	{
		logPrint("Vote Mod: Not loaded, private match vote times exceed 10 seconds. Check vote.gsc\n");
		return;
	}
	else if(!isDefined(votetime) || !isDefined(winnertime))
	{
		logPrint("Vote Mod: Not loaded, no vote times given.  Check vote.gsc\n");
		return;
	}
	//Do not run on a party server as there is no intermission
	/*else if(matchmakingGame())
	{
		logPrint("Vote Mod: Not loaded, incompatible with a party match/server.\n");
		return;
	}*/
	else
	...
	level.VoteVoteTime = 0; //Time available to vote
	level.VoteWinnerTime = 0; //Time the winning map is displayed for
